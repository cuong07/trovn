...
function AddAmenities(){
    ...
    const onSubmit = async()=>{
        if(!selectedFile){return message.error("Vui lòng chọn file")}
        if(Object.keys(errors).length > 0){
            console.log("ERRORS:", errors);
            const keys = Object.keys(errors);
            return message.error(`Nhập sai thông tin: ${errors[keys[0]]?.message}`)
        }
    ...
    return (
        <div className=" flex">
            <Form layout="vertical" >
                ...
                <Button className={{display: isSubmitting ? 'none' : "block"}} type="primary" onClick={onSubmit} loading={isSubmitting} >Thêm</Button>
            </Form>
        </div>
    )
}
